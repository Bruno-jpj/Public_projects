...
from prj_settings import UNKNOWN, FREE, OBSTACLE, BLOCK_SIZE, ROBOT, SPEED, BLACK, BLUE, WHITE, RED, GREEN, MAP_ROBOT_PATH, W, H
...
class Simulation:
    ...
    def spec_move(self, path):
        self.chosen_path = path # (0,0) -> (1,0) -> etc
        #
        for dir_x, dir_y in self.chosen_path:
            new_position = point(self.robot.x + dir_x, self.robot.y + dir_y)
    ...
    for dx, dy in self.chosen_path:
        ...
        # aggiorni davvero la posizione del robot
        self.robot = new_position
